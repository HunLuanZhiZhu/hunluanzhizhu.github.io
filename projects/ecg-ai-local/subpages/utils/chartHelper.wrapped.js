/* Auto-wrapped */
__defineModule("subpages/utils/chartHelper.js", function(module, exports, require) {
// utils/chartHelper.js — 纯Canvas 2D图表绘制工具
// 支持: 饼图(5类分色)、环形图、水平柱状图
// 所有方法接收 canvas 2d context + 数据, 直接绘制

// AAMI 5类标准颜色 (v16: 与全局统一)
var CATEGORY_COLORS = {
  N: '#10B981',  // 翡翠绿 (正常)
  S: '#F59E0B',  // 琥珀 (室上性-警告)
  V: '#E11D48',  // 玫瑰红 (室性-危险)
 ...
 * @param {Object} colors - 可选，自定义颜色映射 {N:'#xxx', S:'#xxx', ...}，不传则用默认
 * @param {String} textColor - 可选，中间文字颜色
 */
function drawDonutChart(ctx, cx, cy, outerRadius, innerRadius, data, centerText, colors, textColor) {
  var colorMap = colors || CATEGORY_COLORS
  var total = 0
  var keys = ['N', 'S', 'V', 'F', 'Q']
  for (var i = 0; i < keys.length; i++) {
    total += (data[keys[i]] || 0)
 ...
 * @param {Object} colors - 可选，自定义颜色映射
 * @param {String} labelColor - 可选，标签文字颜色
 * @param {String} valueColor - 可选，数值文字颜色
 */
function drawBarChart(ctx, x, y, width, barHeight, gap, data, colors, labelColor, valueColor) {
  var colorMap = colors || CATEGORY_COLORS
  var keys = ['N', 'S', 'V', 'F', 'Q']
  var maxVal = 0
  for (var i = 0; i < keys.length; i++) {
  ...
  for (var j = 0; j < keys.length; j++) {
    ...
    // 标签
    ctx.fillStyle = labelColor || '#8E8E93'
    ...
    // 数值
    ctx.fillStyle = valueColor || '#000000'
 ...
 * @param {Object} colors - 可选，自定义颜色映射
 * @param {String} textColor - 可选，文字颜色
 */
function drawLegend(ctx, x, y, itemHeight, data, total, colors, textColor) {
  var colorMap = colors || CATEGORY_COLORS
  var keys = ['N', 'S', 'V', 'F', 'Q']
  for (var i = 0; i < keys.length; i++) {
    var k = keys[i]
