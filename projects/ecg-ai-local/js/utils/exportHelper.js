// utils/exportHelper.js — 历史记录导出工具 (H5 版)
// 由小程序版转化: wx.showActionSheet+文件系统+shareFileMessage → 自定义菜单+Blob下载

import { actionSheet, showToast } from '../ui.js'
...
// 导出历史记录: 菜单选格式 → Blob 下载
export function exportAndShare(history) {
  if (!history || history.length === 0) {
    showToast({ title: '暂无历史记录', icon: 'none' })
    return
  }

  actionSheet({
    itemList: ['导出为 JSON', '导出为 CSV'],
    onSelect: function(tapIndex) {
      if (tapIndex === 0) {
        exportJSON(history)
      } else if (tapIndex === 1) {
        exportCSV(history)
...
function exportJSON(history) {
  ...
  showToast({ title: '已导出', icon: 'success' })
}
